...
export default function CreateTemplateEmbedding() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setShowEmbed(true);
      console.log('Generating Create Template embedding');
    } catch (err) {
    ...
    <div className="space-y-6">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <Card>
            <CardHeader>
              <CardTitle>Create Template Configurator</CardTitle>
              <CardDescription>
                Configure your template authoring embedding parameters
              </CardDescription>
            </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Create Template Embedding</CardTitle>
          ...
          <div className="border-muted-foreground/25 m-6 flex min-h-[600px] items-center justify-center rounded-lg border-2 border-dashed">
            {showEmbed && embedConfig ? (
              <EmbedCreateTemplate
                ...
                css={embedConfig.css}
                onTemplateCreated={(data) => console.log('Template created:', data)}
              />
            ) : watchedValue ? (
